...
- 业务后台：`http://124.222.21.44:3001`
- yyapi 中转站：`http://124.222.21.44:3002/v1`

用户登录后应同步 yyapi token 和模型列表：

- Hermes 使用 `provider: openai-api`，`base_url` 固定为 yyapi。
- OpenClaw 默认使用 yyapi，但允许用户自定义或新增供应商。
- Claude Code 默认使用 yyapi，但允许用户自定义或新增供应商。
- yyapi 模型默认选返回列表里的第一个，不写死具体模型名。
- 如果 yyapi 模型列表变化，用户原模型不存在时自动换成新的第一个模型。
- 用户切到非 yyapi 自定义供应商后，不应被 yyapi 自动刷新覆盖。
...
- 同一个 U 盘换电脑允许使用。
- 客户复制到其他位置后，可按后续策略要求重新激活或限制使用。
- usbId 由后端生成或登记，不要求人工逐个录入。
- 后续可增加制盘脚本：检测 U 盘、复制便携包、读取 U 盘序列号并上传后台。
...
- 没有真实密钥。
- 没有开发机绝对路径。
- 没有旧日志、旧 session、lock/pid 状态。
- `npm run build` 通过。
- Rust 侧关键改动已跑 `cargo check`。
- 如修改打包相关逻辑，已跑 `npm run build:desktop`。

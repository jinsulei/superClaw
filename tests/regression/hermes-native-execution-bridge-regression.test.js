...
test('Hermes user turns are owned by the native Agent runtime', () => {
  ...
  assert.match(store, /Normal user turns are executed by the bundled Hermes Agent/)
  assert.match(store, /if \(!HERMES_NATIVE_EXECUTION_ONLY && isHermesCollaborationCapabilityQuestion\(rawText\)\)/)

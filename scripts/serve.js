...
async function main() {
  ...
  try {
    // 便携优先，其次用户目录
    const portablePaths = [
      path.join(__dirname, '..', 'resources', 'data', '.openclaw', 'openclaw.json'),
      path.join(__dirname, '..', 'src-tauri', 'resources', 'data', '.openclaw', 'openclaw.json'),
    ]
    let cfgPath = portablePaths.find(p => fs.existsSync(p))
    if (!cfgPath) cfgPath = path.join(homedir(), '.openclaw', 'openclaw.json')

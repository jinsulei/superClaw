...
assert.equal(session.messages[1].role, 'assistant', 'first send creates one assistant placeholder')
assert.ok(session.messages[1].id, 'assistant placeholder id is non-empty')
assert.ok(String(session.messages[1].id).includes('req-first'), 'assistant placeholder id is tied to the client request id')
assert.equal(new Set(session.messages.map(m => m.id)).size, session.messages.length, 'first-send message ids are unique')

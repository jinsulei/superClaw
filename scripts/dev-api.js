...
async function fetchReadableUrlContent(rawUrl) {
  ...
  try {
    ...
    lines.push(
      ...
      '当前只读取到了短视频分享页的公开标题、描述、封面或页面文本片段；这不等于完整解析视频。当前仓库已内置 ffmpeg/ffprobe，可做基础媒体处理；但即使开发机系统 PATH 里有 yt-dlp 或 whisper，只要仓库/打包 runtime 未内置，就不能视为 U 盘绿色版可用。即使共享 OCR runtime 已补齐，它也只能识别已取得的图片/截图文字，不能单独生成视频逐字稿、字幕、音频转写、完整分镜或视频帧 OCR。缺 yt-dlp/视频下载器、ASR、字幕提取器和视频抽帧流水线时，短视频/社媒链接必须判定为 metadata_only。',
      '当前未内置完整视频解析工具链，只具备 ffmpeg/ffprobe 基础媒体处理能力，仍缺 yt-dlp/视频下载器、ASR、字幕提取器、视频抽帧与视频帧 OCR；如果缺少口播、字幕和完整画面，请先基于已读字段输出【有限文稿分析】，至少包含选题/标题方向、可能受众、开场钩子推断、卖点或情绪点、可仿写方向；同时输出【有限文稿时间轴】和【几句话文稿推断】，用开场句、承接句、转折句、收束句表达，不编造真实时间戳。',
      '可以输出有限时间轴推断、金句/重点推断、仿写方向和直播口播改写建议，但必须标注为基于公开字段推断。主动反问用户是否要基于现有公开字段仿写一版、提炼金句/卖点、改成直播间口播，或补充逐字稿/截图后继续完整拆解。',
      '不要声称已经读取完整视频正文、逐字稿、字幕、画面 OCR、音频转写、完整分镜或完成完整视频拆解。',
      '如需继续读取，优先使用后台工具读取链接页面可公开访问的信息；只有工具结果明确来自用户授权页面时，才可以说读取了用户授权页面。不要在聊天中展示或播放平台页面。',
      '[/读取限制]',
      '[/短视频页面可读取信息]'
    )
    return lines.join('\n')

...
async function fetchReadableUrlContent(rawUrl) {
  ...
  try {
    ...
    lines.push(
      ...
      '当前只读取到了短视频分享页的公开标题、描述、封面或页面文本片段；这不等于完整解析视频。即使开发机系统 PATH 里有 ffmpeg、yt-dlp 或 whisper，只要仓库/打包 runtime 未内置，就不能视为 U 盘绿色版可用。即使共享 OCR runtime 已补齐，它也只能识别已取得的图片/截图文字，不能单独生成视频逐字稿、字幕、音频转写、完整分镜或视频帧 OCR。没有 ffmpeg、yt-dlp/视频下载器、ASR、字幕提取器和视频抽帧流水线时，短视频/社媒链接必须判定为 metadata_only。',
      '当前未内置完整视频解析工具链，缺 ffmpeg、yt-dlp/视频下载器、ASR、字幕提取器、视频抽帧与视频帧 OCR；如果缺少口播、字幕和完整画面，请先基于已读字段输出【有限文稿分析】，至少包含选题/标题方向、可能受众、开场钩子推断、卖点或情绪点、可仿写方向；同时输出【有限文稿时间轴】和【几句话文稿推断】，用开场句、承接句、转折句、收束句表达，不编造真实时间戳。',

...
assert.match(nativeClaudeBlock, /writeNativeClaudeLauncher\(\{ paths, runCwd \}\)/)
assert.match(nativeClaudeBlock, /spawn\(\s*['"]cmd\.exe['"][\s\S]{0,220}['"]start['"][\s\S]{0,220}launcherPath/)
assert.match(nativeClaudeBlock, /windowsHide:\s*false/)
assert.match(nativeClaudeBlock, /terminalWindow:\s*true/)
assert.match(nativeClaudeBlock, /visible_terminal/)
assert.match(nativeClaudeBlock, /background:\s*false/)
assert.doesNotMatch(nativeClaudeBlock, /spawn\(paths\.claude/)
assert.doesNotMatch(nativeClaudeBlock, /background:\s*true/)
console.log('CLAUDE_TERMINAL_VISIBLE: PASS')

const claudePanelNativeBlock = functionBlock(claudePanelServer, 'handleNativeClaudeStart')
assert.match(claudePanelNativeBlock, /spawn\(\s*["']cmd\.exe["'][\s\S]{0,220}["']start["'][\s\S]{0,220}launcherPath/)
assert.match(claudePanelNativeBlock, /windowsHide:\s*false/)
assert.match(claudePanelNativeBlock, /terminalWindow:\s*true/)
assert.match(claudePanelNativeBlock, /mode:\s*["']visible_terminal["']/)
assert.match(claudePanelNativeBlock, /background:\s*false/)
assert.doesNotMatch(claudePanelNativeBlock, /spawn\(claudeCommand/)
assert.doesNotMatch(claudePanelNativeBlock, /background:\s*true/)
console.log('CLAUDE_PANEL_NATIVE_VISIBLE: PASS')

assert.match(claudePanelServer, /DEFAULT_PORTABLE_HOME/)
assert.match(claudePanelServer, /data["'],\s*["']claude-code["'],\s*["']home["']/)
assert.match(claudePanelServer, /env\.HOME\s*=\s*HOME/)
assert.match(claudePanelServer, /env\.USERPROFILE\s*=\s*HOME/)
console.log('CLAUDE_PANEL_PORTABLE_HOME: PASS')

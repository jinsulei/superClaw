  ...
  _doConnect() {
    ...
    ws.onclose = (e) => {
      ...
      if (e.code === 1008 && !this._intentionalClose) {
        ...
        if (/connect.?failed/i.test(reason) || /not.?ready/i.test(reason) || /unavailable/i.test(reason)) {
          // Gateway 正在初始化（首次启动 sidecars 加载/会话恢复等），短暂重试
          this._initRetryCount++
          // 指数退避：5s → 10s → 15s → 20s → 25s → 30s（上限）
          const delay = Math.min(5000 * this._initRetryCount, 30000)
...
if (!_g.__superclawWsClient) _g.__superclawWsClient = new WsClient()
export const wsClient = _g.__superclawWsClient

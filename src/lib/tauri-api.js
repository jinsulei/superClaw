...
async function invoke(cmd, args = {}) {
  ...
  // Tauri 模式下，WEB_ONLY_CMDS 命令只能走 webInvoke，但打包后无 HTTP 服务器，
  // 此处抛显式错误而非静默回退到 fetch（避免 localhost 连接拒绝页面）
  if (isTauriRuntime()) {
    const errMsg = `[invoke] ${cmd} is WEB_ONLY and unavailable in Tauri build`
    console.warn(errMsg)
...
async function webInvoke(cmd, args) {
  ...
  if (resp.status === 401) {
    ...
    if (!isTauriRuntime() && window.__superclaw_show_login) window.__superclaw_show_login()
...
export async function ensureClaudeCodeStarted() {
  const status = await api.claudeCodeStatus().catch(() => null)
  if (status?.connected || status?.version) {
    const panelUrl = status?.panelUrl || status?.url || status?.panel?.url
    const panelRunning = !!status?.panel?.running
    if (status?.needsPanel || status?.panel?.installed) {
      if (panelRunning && panelUrl) {
        return {
          started: false,
          reused: true,
          mode: 'panel',
      ...
      return api.claudeCodeStart()
    }
    return {
      started: false,
      reused: true,
      mode: status.mode || 'cli',
      url: panelUrl,
      panelUrl,
      status,
    }
...
export const api = {
  ...
  stopAgent: (agent) => invoke('stop_agent', { agent }),
  ...
  collaborationMessageAppend: (message) => invoke('collaboration_message_append', { message }),
  collaborationMessageDrain: (toAgent) => invoke('collaboration_message_drain', { toAgent }),
  fileServiceRun: (agent, sessionId, taskId, operation, inputPath, options = {}) => invoke('file_service_run', {
    agent,
    sessionId,
    taskId,
    operation,
  ...
  getUsbBindingContext: () => invoke('get_usb_binding_context'),
  ...
  assistantOpenPath: (path) => invoke('assistant_open_path', { path }),
  ...
  hermesAgentRunStream: (input, sessionId, conversationHistory, instructions, attachments, onEvent, options) => webStreamInvoke('hermes_agent_run_stream', {
    input,
    sessionId: sessionId || null,
    conversationHistory: conversationHistory || null,
    instructions: instructions || null,
  ...
  hermesVoiceConfigRead: () => invoke('hermes_voice_config_read'),
  hermesVoiceConfigWrite: (config) => invoke('hermes_voice_config_write', { config }),
  hermesVoiceTranscribe: (payload) => invoke('hermes_voice_transcribe', payload || {}),
  hermesVoiceSynthesize: (payload) => invoke('hermes_voice_synthesize', payload || {}),
  ...
  claudeCodeNativeStart: (cwd) => invoke('claude_code_native_start', { cwd: cwd || null }),
  claudeCodeNativeStop: () => invoke('claude_code_native_stop'),
  ...
  hermesSkillInstall: (sourceType, sourceData) => invoke('hermes_skill_install', { sourceType, sourceData }),
  hermesEnsureBuiltinSkills: () => invoke('hermes_ensure_builtin_skills'),

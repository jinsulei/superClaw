...
let _pollTimer = null
let _pollVisibilityHandler = null
const GATEWAY_POLL_INTERVAL_MS = 30000
/** 启动 Gateway 状态轮询（每 30 秒检测一次，页面隐藏时暂停） */
export function startGatewayPoll() {
  ...
  _pollTimer = setInterval(tick, GATEWAY_POLL_INTERVAL_MS)
  if (!_pollVisibilityHandler && typeof document !== 'undefined') {
    _pollVisibilityHandler = () => {
      if (!document.hidden) refreshGatewayStatus()
    }

        ...
        <div style="margin-top:var(--space-sm);padding:10px 12px;background:var(--bg-tertiary);border-radius:var(--radius-sm);font-size:var(--font-size-xs);color:var(--text-secondary);line-height:1.6">
          ...
          ${isMacPlatform()
            ? `${t('setup.macNodeHint')}<br>
               <code style="background:var(--bg-secondary);padding:2px 6px;border-radius:3px;user-select:all">open /Applications/SuperClaw.app</code>`
            : `${t('setup.winNodeHint')}`
          }
          ...
          try {
            ...
            if (config) {
              ...
              if (!config.tools || !config.tools.profile) {
                config.tools = { profile: 'minimal', sessions: { visibility: 'all' }, ...(config.tools || {}) }
                config.tools.profile = 'minimal'
                if (!config.tools.sessions) config.tools.sessions = {}
                config.tools.sessions.visibility = 'all'
                patched = true
                modal.appendHtmlLog(`${statusIcon('ok', 14)} ${t('setup.toolsMinimalEnabled')}`)
              }

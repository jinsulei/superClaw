  ...
  el.innerHTML = `
    ...
    <div class="config-section">
      ...
      <div class="form-group" style="margin-bottom:var(--space-md)">
        ...
        <div class="gw-option-cards">
          <label class="gw-option-card ${(gw.tools?.profile || 'minimal') === 'minimal' ? 'selected' : ''}" data-tools-profile="minimal">
            <input type="radio" name="gw-tools-profile" value="minimal" ${(gw.tools?.profile || 'minimal') === 'minimal' ? 'checked' : ''} hidden>
            <div class="gw-option-icon">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 2L3 14h8l-2 8 10-12h-8l2-8z"/></svg>
            </div>
            <div class="gw-option-text">
              <div class="gw-option-title">${t('gateway.toolsMinimal')}</div>
              <div class="gw-option-desc">${t('gateway.toolsMinimalDesc')}</div>
            </div>
          </label>
          ...
          <label class="gw-option-card ${gw.tools?.profile === 'full' ? 'selected' : ''}" data-tools-profile="full">
            <input type="radio" name="gw-tools-profile" value="full" ${gw.tools?.profile === 'full' ? 'checked' : ''} hidden>
            <div class="gw-option-icon">
...
async function saveConfig(page, state) {
  ...
  const toolsProfile = page.querySelector('input[name="gw-tools-profile"]:checked')?.value || 'minimal'

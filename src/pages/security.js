  ...
  if (isTauri) {
    ...
    if (cmd === 'auth_ignore_risk') {
      if (args.enable) {
        ...
        delete cfg.mustChangePassword
        cfg.ignoreRisk = true
        sessionStorage.removeItem('superclaw_authed')
      } else {
        delete cfg.ignoreRisk
      }
      await api.writePanelConfig(cfg)
    ...
    <div class="config-section">
      ...
      <form id="form-change-pw" style="max-width:400px">
        ${status.hasPassword ? `
          <div style="margin-bottom:12px">
            ...
            <div style="display:flex;gap:6px;align-items:center">
              <input type="password" id="sec-old-pw" class="form-input" placeholder="${t('security.currentPasswordPlaceholder')}" autocomplete="current-password" style="flex:1;min-width:0"
                ${status.defaultPassword ? `value="${status.defaultPassword}"` : ''}>
              <button type="button" class="btn btn-secondary btn-sm" data-toggle-password="#sec-old-pw" title="${t('gateway.show') || '显示'}">${icon('eye', 14)}</button>
            </div>
        ...
        <div style="margin-bottom:12px">
          ...
          <div style="display:flex;gap:6px;align-items:center">
            <input type="password" id="sec-new-pw" class="form-input" placeholder="${t('security.newPasswordPlaceholder')}" autocomplete="new-password" style="flex:1;min-width:0">
            <button type="button" class="btn btn-secondary btn-sm" data-toggle-password="#sec-new-pw" title="${t('gateway.show') || '显示'}">${icon('eye', 14)}</button>
          </div>
        ...
        <div style="margin-bottom:16px">
          ...
          <div style="display:flex;gap:6px;align-items:center">
            <input type="password" id="sec-confirm-pw" class="form-input" placeholder="${t('security.confirmPasswordPlaceholder')}" autocomplete="new-password" style="flex:1;min-width:0">
            <button type="button" class="btn btn-secondary btn-sm" data-toggle-password="#sec-confirm-pw" title="${t('gateway.show') || '显示'}">${icon('eye', 14)}</button>
          </div>
        </div>
...
function bindSecurityEvents(container, status) {
  container.querySelectorAll('[data-toggle-password]').forEach(btn => {
    btn.addEventListener('click', () => {
      const input = container.querySelector(btn.dataset.togglePassword)
      if (!input) return
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // 清除默认密码横幅
        sessionStorage.removeItem('superclaw_must_change_pw')

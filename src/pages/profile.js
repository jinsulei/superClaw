      ...
      <div class="card profile-detail-card">
        ...
        <div class="card-body" style="padding:0">
          ...
          <div class="profile-detail-row">
            ...
            <span class="profile-detail-value">${escapeHtml(formatUserStatus(user?.status))}</span>
          </div>
        </div>
      </div>
        ...
        <div class="card-body" style="padding:16px">
          <div class="profile-token-grid">
            ...
            <div class="profile-token-item">
              <div class="profile-token-label">${t('profile.usedQuota')}</div>
...
function formatUserStatus(status) {
  ...
  if (!value) return '—'
  if (['active', 'normal', 'ok'].includes(value)) return t('profile.statusActive')
  if (['enabled', 'enable'].includes(value)) return t('profile.statusEnabled')
  if (['disabled', 'disable', 'blocked', 'ban', 'banned'].includes(value)) return t('profile.statusDisabled')
  if (['pending', 'inactive'].includes(value)) return t('profile.statusPending')

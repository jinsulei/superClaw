 ...
 *
 * SuperClaw calls the equivalent Rust/Web-stub commands (`hermes_memory_read_all`
 * + `hermes_memory_write`) so the page works on Tauri and Web modes.
 *
 * MEMORY.md and USER.md live in `HERMES_HOME/memories/`; SOUL.md lives at
 * `HERMES_HOME/SOUL.md`, matching the file Hermes loads as its identity.
 */
...
const MEMORY_LINE_TRANSLATIONS = new Map([
  ...
  [
    ...
    '你要清楚沟通，在不确定时承认不确定；除非下方另有要求，优先真正有用，而不是冗长。'
  ],
  [
  ...
  [
    'SuperClaw bridge: when the user asks you to check, open, start, coordinate, command, dispatch, supervise, or make OpenClaw/OpenCloud or CloudCode work, use the local SuperClaw tools instead of only describing what to do.',
    'SuperClaw 桥接：当用户要求检查、打开、启动、协调、指挥、派发、监督或让 OpenClaw/OpenCloud/CloudCode 工作时，优先使用本地 SuperClaw 工具，而不是只描述做法。'
  ],
  [
    'Safety defaults: use read-only CloudCode delegation unless the user explicitly asks to modify files.',
    '安全默认值：除非用户明确要求修改文件，否则 CloudCode 委派使用只读方式。'
  ],
])

function translateMemoryLine(line) {
  const raw = String(line || '')
  const trimmed = raw.trim()
  if (!trimmed) return raw
  if (MEMORY_LINE_TRANSLATIONS.has(trimmed)) {
    return raw.replace(trimmed, MEMORY_LINE_TRANSLATIONS.get(trimmed))
  }
  return raw
    .replace(/^Use this workflow:/, '使用流程：')
    .replace(/^Delegation tools already ensure/, '委派工具已经会确保')
    .replace(/^Do not claim that/, '不要声称')
    .replace(/^Chinese delegation triggers are strict:/, '中文委派触发词必须严格遵守：')
    .replace(/^For opening worker pages,/, '打开工作面板页面时，')
    .replace(/^If the user provides/, '如果用户提供')
}

function bilingualChineseMirror(key, content) {
  ...
  if (key === 'soul') {
    return [
      '## 中文导读',
      '',
      '- 身份：Hermes Agent 是面向执行与协作的智能助手，回答要清楚、直接、真正有用。',
      '- SuperClaw 桥接：涉及 OpenClaw、OpenCloud、CloudCode 的检查、启动、修复、委派或监督时，应优先使用本地桥接工具。',
      '- 桌面优先：抖音、快手、小红书、微信、飞书、钉钉、QQ 等客户端已在任务栏或托盘时，不要直接判定未打开；应先尝试恢复/激活可见窗口。浏览器兜底时只复用一个专用窗口或标签页。',
      '- 短视频读取：用户授权并提供链接时，在后台读取公开可见信息；不展示、不播放、不保存平台页面，也不绕过登录、权限或平台限制。',
      '- 安全默认：没有明确要求修改文件时，CloudCode 委派默认只读；高风险工具需要用户明确授权。',
      '',
      '下面右侧保留英文/原始文档，Hermes 启动时仍按原文读取。'
    ].join('\n')
  }
  if (key === 'memory' && !hasCjk(clean)) {
    return clean
      .split(/\r?\n/)
      .map(translateMemoryLine)
      .join('\n')
  }
  if (key === 'user') {
    const translated = clean
      ...
      .map(translateMemoryLine)
      .join('\n')
    return translated === clean && !hasCjk(clean)
      ? `用户画像原文偏英文，右侧保留原文。中文侧用于快速理解：\n\n${translated}`
      : translated
  }
  return hasCjk(clean) ? clean : clean.split(/\r?\n/).map(translateMemoryLine).join('\n')
}

function renderBilingualContent(section, content) {
  const zh = bilingualChineseMirror(section.key, content)
  const originalLabel = hasCjk(content) ? '原文' : 'English / 原文'
  return `
    <div class="hm-mem-bilingual">
      <div class="hm-mem-lang-card hm-mem-lang-card--zh">
        <div class="hm-mem-lang-label">
          <span>中文</span>
          <small>导读 / 翻译</small>
        </div>
        <div class="hm-mem-rendered hm-mem-rendered--lang markdown-body">${mdToHtml(zh)}</div>
      </div>
      <div class="hm-mem-lang-card hm-mem-lang-card--original">
        <div class="hm-mem-lang-label">
          <span>${originalLabel}</span>
          <small>启动读取内容</small>
        </div>
        <div class="hm-mem-rendered hm-mem-rendered--lang markdown-body">${mdToHtml(content)}</div>
      </div>
    </div>
  `
}
    ...
    return `
      <div class="hm-panel hm-mem-panel hm-mem-panel--${section.key}" data-key="${section.key}">
        ...
        <div class="hm-panel-body">
          ...
          ${content.trim()
            ? renderBilingualContent(section, content)
    ...
    el.innerHTML = `
      <div class="hm-hero">
        <div class="hm-hero-title">
          ...
          <div class="hm-hero-sub">HERMES_HOME/memories + SOUL.md</div>
        </div>

    ...
    el.innerHTML = `
      <div class="hm-hero" data-state="${gwRunning ? 'running' : 'stopped'}">
        <div class="hm-hero-title">
          ...
          <div class="hm-hero-sub">${esc(gatewayUrl)} · ${esc(model)} · ${esc(version)}</div>
        </div>
        ...
        <section class="hm-panel">
          ...
          <div class="hm-panel-body hm-panel-body--tight">
            <div class="hm-services-rows">
              ${renderInfoRow(t('engine.dashVersion'), version)}
      ...
      <section class="hm-panel hm-services-panel" style="margin-top:16px">
        ...
        <div class="hm-panel-body hm-panel-body--tight">
          <div class="hm-pills" style="margin-bottom:12px">
            ...
            ${envData?.wsl2?.available ? `<button class="hm-pill hm-services-mode ${targetMode === 'wsl2' ? 'is-active' : ''}" data-mode="wsl2">WSL2${envData.wsl2.gatewayRunning ? ` · ${esc(t('engine.servicesReadyTag'))}` : ''}</button>` : envData?.wsl2?.wslNoDistros ? `<button class="hm-pill hm-services-mode" disabled style="opacity:0.5;cursor:not-allowed" title="${esc(t('engine.dashWslNoDistros') || '')}">WSL2 ⚠</button>` : ''}
          ...
          ${envData ? `
            <div class="hm-services-env-grid">
              ${envData?.wsl2?.available || envData?.wsl2?.wslNoDistros ? `
                <div class="hm-services-env-card">
                  <div class="hm-services-env-title">WSL2</div>
                  ${envData.wsl2.wslNoDistros ? `
                    <div class="hm-services-env-meta" style="color:var(--hm-warn)">⚠ ${esc(t('engine.dashWslNoDistros') || '')}</div>
                    <div class="hm-services-env-meta" style="color:var(--hm-warn);font-size:11px">${esc(t('engine.dashWslInstallHint') || '')}</div>
                  ` : `
              ...
              ` : ''}
              ${envData?.portable?.available ? `
                <div class="hm-services-env-card">
                  <div class="hm-services-env-title">${esc(t('engine.dashPortableMode'))}</div>
                  ${envData.portable.hasBash ? `<div class="hm-services-env-meta" style="color:var(--hm-cta)">${esc(t('engine.dashPortableBash'))} ${esc(t('engine.dashPortableReady'))}</div>` : ''}
                  ${envData.portable.hasRg ? `<div class="hm-services-env-meta" style="color:var(--hm-cta)">${esc(t('engine.dashPortableRg'))} ${esc(t('engine.dashPortableReady'))}</div>` : ''}
                </div>
              ` : ''}
            </div>
          ` : ''}
  ...
  async function refresh(withSpinner = true) {
    ...
    try {
      ...
      if (healthResult.status === 'fulfilled') {
        ...
        health = null
        setPageMessage(stripError(healthResult.reason), 'warn')
      }

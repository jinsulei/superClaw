...
function aggregateSessions(sessions) {
  ...
  // 规范化模型名：去掉 provider/ 前缀（如 "anthropic/claude-opus-4.6" → "claude-opus-4.6"）
  function normalizeModel(name) {
    if (!name) return t('usage.unknownModel')
...
function renderContent(usage, configModel) {
  ...
  const modelRows = usage.modelUsage.length
    ? usage.modelUsage.slice(0, 10).map(model => {
        const isCurrent = configModel && model.model === configModel
        const rowClass = isCurrent ? 'is-current' : ''
        const currentTag = isCurrent
          ? ' <span class="hm-usage-current-tag">' + escHtml(t('usage.currentTag')) + '</span>'
          : ''
        return `<div class="hm-usage-model-row ${rowClass}">
          <div class="hm-usage-model-name" title="${escHtml(model.model)}">${escHtml(model.model)}${currentTag}</div>
...
export function render() {
    ...
    el.innerHTML = `
      <section class="hm-usage-hero">
        <div class="hm-usage-hero-copy">
          ...
          ${configModel ? `<div class="hm-usage-model-badge">${escHtml(t('usage.currentModel'))}: <strong>${escHtml(configModel)}</strong></div>` : ''}
        </div>
      ...
      <div class="hm-usage-body">
        ...
        ` : renderContent(usage, configModel)}
      </div>
    `
  ...
  async function load() {
    ...
    try {
      ...
      if (alive) configModel = cfg?.model || cfg?.model_raw || ''
    } catch (_) { /* 静默失败，不阻塞使用量加载 */ }
    try {

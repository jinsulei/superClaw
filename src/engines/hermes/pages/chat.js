...
function formatVideoMaterialCompletenessRules(platform = '短视频平台') {
  return [
    ...
    '回答结构必须包含：1. 结论：当前不是模型不会回答，而是素材不足；2. 已获取素材：标题、作者、描述、话题、互动数据、热门评论、页面公开字段；3. 未获取素材：视频逐字稿、字幕、音频转写、画面 OCR、完整视频正文、分镜内容；4. 为什么受限：当前版本没有稳定视频解析器/逐字稿提取器，只能基于公开字段做有限分析；5. 有限文稿分析：基于已获取的标题/描述/话题/关键词输出选题方向、可能受众、开场钩子推断、卖点或情绪点、可仿写方向；6. 有限文稿时间轴：没有逐字稿时不要编造真实时间戳，只输出开场句、承接句、转折句、收束句等几句话文稿推断，并标注“基于公开字段推断”；7. 还能做什么：选题判断、评论反馈分析、账号内容定位、粗略卖点拆解、爆点方向判断；8. 用户可以补充：视频逐字稿、字幕、口播文本、视频截图、商品信息或链接正文；9. 下一步建议：补充素材后继续完整拆解。',

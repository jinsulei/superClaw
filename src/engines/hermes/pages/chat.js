...
function formatShortVideoWorkflowInstructions(platform = '短视频平台') {
  return [
    ...
    '如果后台读取工具无法读取该链接的视频正文、字幕、口播或逐字稿，要说明是平台跳转、登录/反爬限制、页面权限或缺少专用视频解析器导致没有拿到完整素材；不要把失败原因写成用户桌面端状态同步失败，也不要说“用户没有在隔离浏览器里打开过”。',
    ...
    '失败时的优先动作是“后台公开信息读取”：如果工具列表里有 browser、browser_navigate、browser_snapshot、computer_use 或类似浏览器工具，先尝试读取链接页面可公开访问的信息；只有工具结果明确来自用户授权页面时，才可以说读取了用户授权页面。',
    ...
    '如果浏览器读取工具不可用，或页面读取仍失败，再用中文说明“暂时拿不到视频正文”；如果已经拿到标题、描述、话题、关键词、评论或页面公开字段，必须先给“有限文稿分析”，再给素材补充模板，不能只给模板。素材补充只要画面描述、字幕/口播、核心观点，不要主动要求截图。不要问“你觉得哪条路方便”。',
    ...
    '7. 可复用核心与可优化点',
    '',
    '如果最终仍缺少素材但已有标题/描述/话题/关键词，请先输出【有限文稿分析】：',
    '1. 选题/标题方向：基于标题和话题判断内容主题，不编造画面。',
    '2. 可能受众：基于关键词、平台和内容类型判断。',
    '3. 开场钩子推断：只能从标题/描述推断，不写成确定口播。',
    '4. 卖点或情绪点：基于公开字段提炼。',
    '5. 可仿写方向：给标题、开场、口播结构或发布文案的可执行方向。',
    '同时输出【有限文稿时间轴】和【几句话文稿推断】：没有逐字稿时不要编造真实时间戳，只能用“开场句/承接句/转折句/收束句”的结构化推断，逐句标注“基于公开字段推断”。',
    '',
    '然后给用户一个可直接粘贴的素材模板：',
    '【视频素材】',
    `平台：${platform}`,
...
function formatVideoMaterialCompletenessRules(platform = '短视频平台') {
  return [
    ...
    'Portable toolchain status: bundled yt-dlp, ffmpeg, ffprobe, whisper.cpp CLI, and ggml-tiny model are available from src-tauri/resources/runtime/video-tools. Runtime code must prefer these bundled paths over system PATH or Python-installed launchers.',
    'Important limit: bundled tools do not guarantee every Douyin/Xiaohongshu/Kuaishou link can be parsed. Some platforms require fresh cookies, login state, anti-bot handling, or user-provided media. If yt-dlp reports cookies/login required, explain that the downloader is available but platform access is blocked.',
    '便携版判断：当前仓库已内置 ffmpeg/ffprobe，可做基础媒体处理；但即使开发机系统 PATH 里有 yt-dlp 或 whisper，只要仓库/打包 runtime 未内置，就不能视为 U 盘绿色版可用；portableReady 必须保持 false。',
    '能力边界：OCR runtime 只能识别已取得的图片/截图文字；ffmpeg/ffprobe 只能提供基础媒体处理能力，不能单独下载平台视频，也不能单独生成视频逐字稿、字幕、音频转写或视频分镜。缺 yt-dlp/视频下载器、ASR、字幕提取器和视频抽帧流水线时，短视频/社媒链接必须判定为 metadata_only；如果这些工具只在系统 PATH 中存在、没有仓库内置，则绿色版仍按缺失处理。',
    '如果用户问“为什么素材不足”“为什么只回答一半”“还能分析什么”，必须用中文完整说明，不能只回一句“只能拿到公开字段”。',
    '回答结构必须包含：1. 结论：当前不是模型不会回答，而是素材不足；2. 已获取素材：标题、作者、描述、话题、互动数据、热门评论、页面公开字段；3. 未获取素材：视频逐字稿、字幕、音频转写、画面 OCR、完整视频正文、分镜内容；4. 为什么受限：当前版本没有稳定视频解析器/逐字稿提取器，当前只有 ffmpeg/ffprobe 基础媒体处理能力，当前未内置完整视频解析工具链，仍缺 yt-dlp/视频下载器、ASR、字幕提取器、视频抽帧与视频帧 OCR，当前只能基于公开字段做有限分析；5. 有限文稿分析：基于已获取的标题/描述/话题/关键词输出选题方向、可能受众、开场钩子推断、卖点或情绪点、可仿写方向；6. 有限文稿时间轴：没有逐字稿时不要编造真实时间戳，只输出开场句、承接句、转折句、收束句等几句话文稿推断，并标注“基于公开字段推断”；7. 还能做什么：选题判断、评论反馈分析、账号内容定位、粗略卖点拆解、爆点方向判断、有限时间轴推断、金句/重点推断、直播口播改写；8. 用户可以补充：视频逐字稿、字幕、口播文本、视频截图、商品信息或链接正文；9. 下一步建议：主动反问用户是否要基于现有公开字段仿写一版、提炼金句/卖点、改成直播间口播，或补充逐字稿/截图后继续完整拆解；补充素材后继续完整拆解。',
    '只要已获取标题、描述、话题、关键词、评论或页面公开字段，就不能只输出素材补充模板；必须先输出【有限文稿分析】，并明确这些分析是基于公开字段的推断。',
    '有限文稿时间轴、几句话文稿推断、金句/重点推断都必须标注为“基于公开字段推断”，不能写成真实逐字稿或真实时间戳。',
    '禁止声称已经完整读取视频、已经解析视频正文、已经获取逐字稿、已经完成视频拆解、已经完成视频 OCR、已经拿到完整分镜、已经完成完整视频拆解，除非前置内容明确包含完整 transcript/captions/subtitles/OCR/audio transcript。',
    '[/MATERIAL_COMPLETENESS_REQUIREMENT]',
  ]
}
...
function formatVideoLinkAnalysisRequest(url, fetchedContent = '') {
  ...
  const lines = [
    ...
    `平台: ${platform}`,
    `URL: ${url}`,
    '来源: 用户通过加号入口或聊天消息提交视频/社媒链接。当前版本不会直接解析视频正文、画面或字幕，只会把链接交给 Hermes 进行文本化分析；如需完整读取视频内容，后续需要接入专门解析器，或由工具明确读取到用户授权页面的公开可见信息。',

...
// Playwright downloads are generated by this panel and are safe to expose as output artifacts.
const BROWSER_OUTPUT_DIR = path.join(APP_CONFIG_DIR, "browser-output");
...
const TOOL_PROFILES = {
  ...
  audit: ["Glob", "Grep", "Read", "LS", "Bash", "BashOutput", "KillBash"],
...
const TAKEOVER_EXECUTION_SYSTEM_PROMPT = [
  "This run is in SuperClaw takeover mode and the user has already confirmed it.",
  "The current task may use the provided local read, edit, write, and command tools together with the authorized browser tools. Do not claim Bash, Write, or browser automation are unavailable when they appear in the current tool list.",
  "Use the least invasive tool that completes the task. Before deleting, overwriting, bulk-writing, installing dependencies, logging in, uploading files, submitting private data, paying, or changing remote state, explain the action and request a separate confirmation.",
  "Never read credentials, private keys, tokens, or .env files unless the user explicitly requests that exact sensitive action and confirms it.",
].join("\n");
...
function processNarrativeSummary(text) {
  const value = String(text || "").trim();
  if (!value) return "";
  if (containsReasoningLeakText(value)) {
    return "正在分析任务目标、已有上下文和下一步操作。";
  }
  return redact(stripUnsupportedToolCallText(value)).trim();
}
...
const PANEL_DOCUMENT_EXTENSIONS = new Set([".pdf", ".doc", ".docx", ".xls", ".xlsx", ".csv", ".ppt", ".pptx"]);

function findPortableDocumentByName(fileName, roots) {
  const expected = String(fileName || "").toLowerCase();
  const matches = [];
  const visit = (directory, depth = 0) => {
    if (matches.length > 1 || depth > 4) return;
    let entries = [];
    try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          // A file can disappear while a historical conversation is rendering.
        }
      }
    }
  };
...
function resolvePanelDocumentPath(rawPath) {
  const requested = path.resolve(String(rawPath || ""));
  const extension = path.extname(requested).toLowerCase();
  if (!PANEL_DOCUMENT_EXTENSIONS.has(extension)) throw new Error("仅支持打开或下载文档附件");
  if (containsSensitiveFileName(path.basename(requested))) throw new Error("敏感文件不能通过面板打开或下载");
  const roots = [UPLOAD_DIR, BROWSER_OUTPUT_DIR, ...getExecutionRoots()]
    .filter((root) => fs.existsSync(root))
    .map((root) => fs.realpathSync(root));
  const target = fs.existsSync(requested) && fs.statSync(requested).isFile()
    ? fs.realpathSync(requested)
    : findPortableDocumentByName(path.basename(requested), roots);
  if (!target) throw new Error("文件不存在或已被移动");
  if (!roots.some((root) => isSameOrInside(target, root))) {
    throw new Error("该文件不在当前项目或便携附件目录内");
  }
  return target;
}

function documentContentType(filePath) {
  const types = {
    ".pdf": "application/pdf",
...
function documentAttachmentDisposition(filePath) {
  const fileName = path.basename(filePath).replace(/[\\\r\n"]/g, "_");
  // Node rejects non-Latin-1 characters in a header value. Keep a safe fallback
  // for older clients and carry the original Chinese name through RFC 5987.
...
function handlePanelDocumentDownload(res, url) {
  try {
    const target = resolvePanelDocumentPath(url.searchParams.get("path"));
    res.writeHead(200, {
      "content-type": documentContentType(target),
      "content-length": fs.statSync(target).size,
      "content-disposition": documentAttachmentDisposition(target),
      "cache-control": "no-store",
    });
    fs.createReadStream(target).pipe(res);
  } catch (error) {
    sendJson(res, 400, { error: error?.message || "文件不可下载" });
  }
}

function handlePanelDocumentOpen(req, res) {
  readRequestBody(req).then((payload) => {
    const target = resolvePanelDocumentPath(payload?.path);
    if (process.platform === "win32") {
    ...
    appendAuditLog({ feature: "/api/local-document/open", action: "user-open-document", projectPath: target, result: "opened" });
    sendJson(res, 200, { ok: true, name: path.basename(target) });
  }).catch((error) => sendJson(res, 400, { error: error?.message || "文件无法打开" }));
}
...
function browserAutomationAllowed(payload, toolProfile) {
  if (!["none", "read", "audit", "edit", "command"].includes(toolProfile)) return false;
...
function findNamedUploadedFilesForOpenRequest(prompt) {
  ...
  const requestedNames = new Set(
    Array.from(request.matchAll(/([^\\/:*?"<>|\s]+\.(?:txt|md|json|csv|pdf|docx?|xlsx?|pptx|png|jpe?g|webp|gif|mp4))/gi))
...
function attachmentPromptBlock(attachments) {
  ...
  return [
    ...
    "For .docx, .xlsx, .pptx, and .pdf attachments, call mcp__superclaw_local_desktop__inspect_local_document first. It is the shared offline, read-only document service and works in the portable package.",
    "Use the inspected content as evidence. If the shared document service fails, state the failure clearly; do not claim to have read content that was not successfully inspected.",
    ...lines,
    "[/Claude Code native attachment bridge]",
  ].join("\n");
}
...
async function handleRun(req, res) {
  ...
  if (runAttachments.length && toolProfile !== "edit" && toolProfile !== "command") {
  ...
  if (!toolProfile) {
    sendJson(res, 400, { error: "toolProfile 只能是 none、read、audit、edit 或 command" });
    return;
  }
  ...
  if (payload.permissionProfile === "takeover") {
    args.push("--append-system-prompt", TAKEOVER_EXECUTION_SYSTEM_PROMPT);
  }
  if (allowBrowserAutomation) {
    args.push("--append-system-prompt", BROWSER_AUTOMATION_SYSTEM_PROMPT);
    args.push(
      "--append-system-prompt",
      `Browser automation downloads are saved to this deterministic SuperClaw output directory: ${BROWSER_OUTPUT_DIR}. This is a controlled panel artifact directory, not an unknown path. Report its full path when returning generated files; do not claim that you have no local read capability when Read/Glob/LS are present in the current tool list.`
    );
  }
  ...
  } else if (toolProfile === "audit") {
    args.push(
      "--append-system-prompt",
      `本次是只读审计运行：允许读取当前项目目录 ${cwd}、执行只读诊断命令并通过 WebSearch/WebFetch 查询公开网络。禁止 Edit、Write、MultiEdit、删除、移动、重命名、重定向输出、脚本生成文件、安装依赖、修改配置、上传文件或访问敏感文件。命令只可用于检查、搜索、解析、版本查询、测试和验证；不确定命令是否会写入时，不要执行。`
    );
  ...
  function handleJsonLine(line) {
    ...
    if (parsed.type === "assistant") {
      ...
      if (toolBlocks.length) {
        const executionNarrative = [pendingAssistantText, turnText]
          .filter(Boolean)
          .join("\n");
        if (executionNarrative) {
          writeClaudeProcessEvent(res, {
            kind: "reasoning",
            title: "Claude 正在分析",
            text: processNarrativeSummary(executionNarrative),
          });
        }
      ...
      if (turnText) {
        if (containsReasoningLeakText(turnText)) {
          writeClaudeProcessEvent(res, {
            kind: "reasoning",
            title: "Claude 正在分析",
            text: processNarrativeSummary(turnText),
          });
          partialTextSeenForAssistant = false;
          return;
        }
        // A native turn can emit a plain planning envelope immediately before
        // a later tool envelope. Hold it until we know whether it is the
        // final answer or execution narration, so one task never renders as
        // two assistant replies.
        pendingAssistantText = [pendingAssistantText, sanitizeModelOutput(turnText, { prompt })]
          .filter(Boolean)
          .join("\n");
        partialTextSeenForAssistant = false;
        return;
    ...
    if (parsed.type === "result") {
      if (bufferedAssistantDeltas) {
        const visibleText = sanitizeModelOutput(bufferedAssistantDeltas, { prompt });
        if (visibleText) {
          assistantTextSeen = true;
          writeEvent(res, "text", { text: visibleText });
        }
        bufferedAssistantDeltas = "";
        ...
        const visibleText = sanitizeModelOutput(parsed.result, { prompt });
        if (visibleText) writeEvent(res, "text", { text: visibleText });
      }
      writeEvent(res, "done", {
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/api/local-document") {
    handlePanelDocumentDownload(res, url);
    return;
  }
  if (req.method === "POST" && url.pathname === "/api/local-document/open") {
    handlePanelDocumentOpen(req, res);
    return;
  }

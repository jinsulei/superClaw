...
function createBrowserTool(opts) {
	...
	return {
		...
		description: [
			...
			"Single-tab rule: before opening a URL, reuse the existing browser tab with action=navigate/focus. Do not create multiple tabs for one task.",
		...
		execute: async (_toolCallId, args) => {
			...
			switch (action) {
				...
				case "open": {
					...
					if (proxyRequest) {
						const existingTabs = (await proxyRequest({
							method: "GET",
							path: "/tabs",
							profile,
							timeoutMs: toolTimeoutMs
						})).tabs ?? [];
						const firstTab = Array.isArray(existingTabs) ? existingTabs[0] : null;
						const reusableTargetId = readStringValue(firstTab?.suggestedTargetId) ?? readStringValue(firstTab?.tabId) ?? readStringValue(firstTab?.targetId);
						if (reusableTargetId) return jsonResult(await proxyRequest({
							method: "POST",
							path: "/navigate",
							profile,
							body: {
						...
						const existingTabs = await browserToolDeps.browserTabs(baseUrl, {
							profile,
							timeoutMs: toolTimeoutMs
						}).catch(() => []);
						const firstTab = Array.isArray(existingTabs) ? existingTabs[0] : null;
						...
						if (reusableTargetId) {
							...
							touchTrackedTab(readStringValue(result.targetId) ?? reusableTargetId);
							return jsonResult(result);
						}
					}
					if (proxyRequest) return jsonResult(await proxyRequest({

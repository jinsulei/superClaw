...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        agent = AIAgent(
            ...
            **runtime_kwargs,
            ...
            clarify_callback=self._api_server_clarify_callback,
    ...
    def _api_server_clarify_callback(self, question, choices):
        """Non-blocking clarify fallback for /v1/runs.

        Messaging adapters and the CLI have an interactive clarify UI. The
        embedded SuperClaw chat uses SSE runs, where blocking the worker thread
        waiting for a separate user response would freeze the turn unless the
        frontend explicitly implements a clarify reply flow. Return a clear
        fallback answer so the tool remains callable and the model continues
        instead of seeing "Clarify tool is not available in this execution
        context.".
        """
        try:
            logger.info(
                "api_server clarify requested question=%r choices=%r",
                str(question)[:500],
                choices,
            )
        except Exception:
            pass
        return (
            "当前 SuperClaw 聊天未开启阻塞式澄清弹窗；用户没有提供额外补充。"
            "请基于现有上下文继续完成任务。若信息确实不足，请在最终回复中直接说明还需要哪些具体信息。"
        )
    ...
    def _make_run_event_callback(self, run_id: str, loop: "asyncio.AbstractEventLoop"):
        ...
        def _callback(event_type: str, tool_name: str = None, preview: str = None, args=None, **kwargs):
            ...
            if event_type == "tool.started":
                ...
                # Keep the structured result on the run SSE event.  The
                # desktop renderer uses this payload for current-chat media
                # returned by tools; dropping it makes a successful tool call
                # indistinguishable from a plain-text "sent" response.

...
fn bundled_cli_path() -> Option<std::path::PathBuf> {
    let bin_dir = crate::commands::bundled_openclaw_bin_dir()?;
    #[cfg(target_os = "windows")]
    let cli = bin_dir.join("openclaw.cmd");
    #[cfg(not(target_os = "windows"))]
    let cli = bin_dir.join("openclaw");
    if cli.exists() {
        Some(cli)
    } else {
        None

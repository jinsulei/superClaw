...
pub async fn check_weixin_plugin_status() -> Result<Value, String> {
    ...
    if installed {
        ...
        if oc_nums < vec![2026, 3, 22] {
            compatible = false;
            compat_error = format!(
                "插件版本与当前 OpenClaw {} 不兼容（要求 >= 2026.3.22），请先升级 OpenClaw 或执行: {} plugins install @tencent-weixin/openclaw-weixin@latest",
                oc_ver,
                crate::utils::openclaw_cli_path_display()
            );
        }
    }
        ...
        let weixin_spec = if version.as_deref().is_some_and(|v| !v.is_empty()) {
            ...
            if needs_legacy {
                ...
                let _ = app.emit(
                    "channel-action-log",
                    json!({ "platform": &platform, "action": &action, "kind": "info",
                        "message": format!("  → {} plugins install @tencent-weixin/openclaw-weixin@latest", crate::utils::openclaw_cli_path_display()) }),
                );
                let _ = app.emit(
                    "channel-action-log",
                    json!({ "platform": &platform, "action": &action, "kind": "info",

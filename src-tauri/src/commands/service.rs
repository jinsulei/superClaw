...
mod platform {
    ...
    pub async fn start_service_impl(_label: &str) -> Result<(), String> {
        ...
        // Auto-fix workspace path for portability (fix stale/relative paths from packaging)
        // If agents.defaults.workspace is empty, relative ("./workspace"), or non-absolute,
        // rewrite it to the correct absolute path for this machine.
        {
                ...
                if let Ok(content) = std::fs::read_to_string(&config_path) {
                    if let Ok(mut config) = serde_json::from_str::<serde_json::Value>(&content) {
                        let default_workspace =
                            openclaw_dir.join("workspace").to_string_lossy().to_string();
                        let needs_fix = config
                            ...
                            .map(|ws| {
                                ws.is_empty()
                                    || ws == "./workspace"
                                    || ws == ".\\workspace"
                                    || !std::path::Path::new(ws).is_absolute()
                            })
                            .unwrap_or(true);
                        if needs_fix {
                            if let Some(agents) =
                                config.get_mut("agents").and_then(|a| a.as_object_mut())
                            {
                                if let Some(defaults) =
                                    agents.get_mut("defaults").and_then(|d| d.as_object_mut())
                                {
                                    defaults.insert(
                                        "workspace".into(),
                                        serde_json::Value::String(default_workspace),
                                    );
                                    let _ = std::fs::write(
                                        &config_path,
                                    ...
                                    log::info!(
                                        "[portable] Auto-fixed workspace to {:?}",
                                        defaults
                                            .get("workspace")
                                            .and_then(|v| v.as_str())
                                            .unwrap_or("?")
